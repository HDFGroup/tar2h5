...
int main(int argc, char** argv) 
{
    ...
    sprintf(h5name, "%s.shredder.h5", tarname);
    printf("|    %s\n", tarname);
    printf("|    %s\n", h5name);
    ...
    assert((image = H5Dcreate(file, "image", H5T_NATIVE_UINT8, fspace, H5P_DEFAULT, dcpl, H5P_DEFAULT)) >= 0);
    assert((image_offset = H5Dcreate(file, "image_offset", H5T_NATIVE_HSIZE, fspace, H5P_DEFAULT, dcpl, H5P_DEFAULT)) >= 0);
    assert((name_offset = H5Dcreate(file, "name_offset", H5T_NATIVE_HSIZE, fspace, H5P_DEFAULT, dcpl, H5P_DEFAULT)) >= 0);
    assert(H5Pset_deflate(dcpl, 6) >= 0);
    assert((name = H5Dcreate(file, "name", H5T_NATIVE_UINT8, fspace, H5P_DEFAULT, dcpl, H5P_DEFAULT)) >= 0);
    ...
    while (archive_read_next_header(a, &entry) == ARCHIVE_OK) 
    {
        for (;;) 
        {
            ...
            if (flg == 1) printf("WARNING: File > 64K found.\n");
            ...
            assert(H5Sselect_hyperslab(fspace, H5S_SELECT_SET, &start, NULL, &one, &block) >= 0);
            assert(H5Dwrite(image, H5T_NATIVE_UINT8, mspace, fspace, H5P_DEFAULT, buff) >= 0);
            assert(H5Sclose(fspace) >= 0);
            ...
            assert(H5Sselect_hyperslab(fspace, H5S_SELECT_SET, &start, NULL, &one, &block) >= 0);
            assert(H5Dwrite(image_offset, H5T_NATIVE_HSIZE, mspace, fspace, H5P_DEFAULT, &offset) >= 0);
            assert(H5Sclose(fspace) >= 0);
            assert(H5Sclose(mspace) >= 0);
            ...
            assert(H5Sselect_hyperslab(fspace, H5S_SELECT_SET, &start, NULL, &one, &block) >= 0);
            assert(H5Dwrite(name, H5T_NATIVE_UINT8, mspace, fspace, H5P_DEFAULT, archive_entry_pathname(entry)) >= 0);
            assert(H5Sclose(fspace) >= 0);
            assert(H5Sclose(mspace) >= 0);
            ...
            assert(H5Sselect_hyperslab(fspace, H5S_SELECT_SET, &start, NULL, &one, &block) >= 0);
            assert(H5Dwrite(name_offset, H5T_NATIVE_HSIZE, mspace, fspace, H5P_DEFAULT, &noffset) >= 0);
            assert(H5Sclose(fspace) >= 0);
            assert(H5Sclose(mspace) >= 0);
